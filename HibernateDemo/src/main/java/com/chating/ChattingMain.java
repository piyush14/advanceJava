...
//		}
		 ...
		 for (ChatRoom list : chatRoomList){
			// Users userList = list.
			 if(chatRoomName.equals(list.toString())){
				 ...
				 sendMessages(list.getChatRoomName());
	...
	public static void sendMessages(String chatRoomName){
		...
		Query query = session.createQuery("from ChatRoom ");
		 ...
		 Messages user = new Messages(msg);
		 user.setMessageData(msg);
		 myList.setMessages(new HashSet<Messages>());
		 myList.getMessages().add(user);
		 session.save(user);
		 transaction.commit();
		 session.close();
	}
